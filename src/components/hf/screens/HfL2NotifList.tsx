/**
 * Notification inbox list. Reuses the `Notif` row primitive from the
 * design (hf-screens-B.jsx lines 517-544) and the segmented filter
 * pattern (全部 / 未读 / 今天 / 拍我).
 *
 * Note: the design file's `window.HfL2Notif` (hf-screens-L2.jsx lines
 * 982-1080) is actually the *notification preferences* settings page
 * (DND, sound, poke caps). That screen lives in `HfL2Notif.tsx`. This
 * file is the separate inbox list used by `/app/me/notifications`.
 *
 * className + inline styles + structure preserved byte-for-byte from
 * the design's `Notif` row. Existing testids (`inbox-list`,
 * `inbox-empty`, `inbox-row-${id}`, `inbox-row-${id}-link`,
 * `inbox-row-${id}-mark-read`, `inbox-mark-all`) are preserved so the
 * existing E2E suite keeps passing.
 */
...
export interface HfL2NotifListProps {
  items: HfNotifItem[];
